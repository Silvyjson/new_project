...
<main class="min-h-screen bg-surface">
        ...
        <div class="absolute inset-0 flex items-center">
            <div class="container max-w-7xl mx-auto px-4">
                <div class="grid md:grid-cols-2 gap-8 items-center">
                    ...
                    <div class="flex items-start gap-6 animate-fade-in">
                        ...
                        <div>
                            <div class="inline-flex gap-1 items-center">
                                <h1 class="text-[32px] md:text-[40px] font-bold text-white mb-2">
                                    {shop.name}
                                </h1>
                                <button
                            ...
                            <div class="flex gap-3">
                                <Button href="/shop/{shop.slug}/product" variant="primary" size="md">Shop Now</Button>
                                <button
                    ...
                    <!-- Right: Trust Metrics -->
                    <div class="hidden md:block animate-fade-in" style="transition-delay: 100ms">
                        <Card className="bg-surface/95 backdrop-blur-sm p-6 max-w-md ml-auto">
                            <div class="grid grid-cols-2 gap-4">
                                <div class="text-center">
                                    <div class="text-[32px] font-bold {getTrustScoreColor(shop.trustScore)}">
                                        {shop.trustScore}%
                                    </div>
                                    <div class="text-small text-text-muted">Trust Score</div>
                                </div>
                                <div class="text-center">
                                ...
                                <div class="text-center">
                                    ...
                                    <div class="text-small text-text-muted">Orders</div>
                                </div>
                                <div class="text-center">
                                    <div class="text-[32px] font-bold text-text-main">
                                        {shop.followers.toLocaleString()}
                                    </div>
                                    <div class="text-small text-text-muted">Followers</div>
                                </div>
                            </div>
                        </Card>
                    </div>
    ...
    <section class="sticky top-[70px] z-40 bg-surface border-b border-gray-200">
        <div class="container max-w-7xl mx-auto px-4">
            <div class="flex items-center justify-between">
                ...
                <a href="/shop/{shop.slug}/product" class="hidden md:flex items-center gap-2 text-primary font-medium hover:underline whitespace-nowrap">
                    View All Products <Icon icon="mdi:arrow-right" class="w-4 h-4" />
                </a>
            </div>
        </div>
    </section>
    ...
    <div class="md:hidden my-2 text-right px-4">
        <a href="/shop/{shop.slug}/product" class="inline-flex items-center gap-2 text-primary font-medium hover:underline">
        ...
        <div class="container max-w-7xl mx-auto px-4">
            ...
            {:else}
                <Card className="py-16 text-center">
                    ...
                    <h3 class="text-h3 text-text-main mb-2">No Products in This Category</h3>
                    <p class="text-body text-text-muted mb-6">Try selecting a different tab or view all products.</p>
                    <Button variant="outline" href="/shop/{shop.slug}/product">View All Products</Button>
                </Card>
            {/if}
        </div>
    </section>
    ...
    <section id="about" class="py-20 bg-surface">
        <div class="container max-w-7xl mx-auto px-4 grid md:grid-cols-2 gap-12">
            ...
            <div class="animate-fade-in">
                <h2 class="text-h3 md:text-h2 text-text-main mb-6">About {shop.name}</h2>
                <p class="text-body text-text-muted leading-relaxed mb-6">{shop.description}</p>
                ...
                <!-- Location -->
                <div class="flex flex-col md:flex-row md:items-center gap-2 md:gap-6 text-body text-text-muted">
                    <span class="flex items-center gap-2"><Icon icon="mdi:map-marker" class="w-4 h-4" /> {shop.location}</span>
                </div>

                <div class="flex gap-3 mt-4">
                    <Button href="/messages?shop={shop.slug}" variant="primary" size="lg" className="flex items-center gap-2">
                        <Icon icon="simple-icons:whatsapp" class="w-6 h-6" /> Message
                    </Button>
                    <Button href="/report?shop={shop.slug}" variant="outline" size="lg" className="flex items-center gap-2 text-error hover:bg-error/5">
            ...
            <div class="animate-fade-in" style="transition-delay: 100ms">
                <Card className="p-6 border border-gray-200 h-full">
                    <h3 class="text-h3 text-text-main mb-6">Shop Information</h3>
                    <div class="space-y-4">
                        {#each [
                            { label: "Response Time", value: shop.stats.responseTime }, 
                            { label: "Processing Time", value: shop.stats.processingTime }, 
                            { label: "Shipping", value: shop.stats.shippingTime }, 
                            { label: "Returns", value: shop.stats.returnPolicy }, 
                            { label: "Trust Score", value: `${shop.trustScore}%`, class: getTrustScoreColor(shop.trustScore) }
                        ] as item}
                            <div class="flex items-center justify-between py-3 border-b border-gray-100 last:border-0">
                                <span class="text-body text-text-muted">{item.label}</span>
                                <span class="text-body font-semibold {item.class || 'text-text-main'}">{item.value}</span>
                            </div>
                        {/each}
                    </div>
                </Card>
    ...
    <section id="reviews" class="py-20 bg-background-light">
        <div class="container max-w-7xl mx-auto px-4">
            <div class="text-center mb-12">
                <h2 class="text-h3 md:text-h2 text-text-main mb-4">Customer Reviews</h2>
                <div class="flex items-center justify-center gap-4 mb-4">
                    <span class="flex items-center gap-2 text-[48px] font-bold text-text-main">
                        <Icon icon="mdi:star" class="text-yellow-400" />{shop.rating}
                    </span>
                    <div class="text-left">
                        <div class="text-body text-text-muted">Based on {shop.reviewCount} reviews</div>
                        <div class="flex items-center gap-1">
                            {#each Array(5) as _, i}
                                <Icon icon="mdi:star" class={i < Math.floor(shop.rating) ? "text-yellow-400" : "text-gray-300"} />
                            {/each}
                        </div>
                    </div>
                </div>
