...
<script lang="ts">
  ...
  const followedShops = [
    { id: 1, name: 'TechStoreNG', logo: '🏪', banner: 'bg-blue-100', newProducts: 12, trustScore: 94 },
    { id: 2, name: 'Amina Fashion', logo: '👗', banner: 'bg-pink-100', newProducts: 8, trustScore: 92 },
    { id: 3, name: 'Home Essentials', logo: '🏠', banner: 'bg-green-100', newProducts: 5, trustScore: 89 }
  ];
...
<div class="max-w-[1280px] mx-auto px-4 py-8">
  ...
  <section class="mb-12 animate-fade-in" style="transition-delay: 100ms">
    <div class="flex items-center justify-between mb-6">
      ...
      <a href="/following" class="text-sm font-medium text-primary hover:underline">View All →</a>
    </div>
          ...
          <Card hover={true} padding="none" className="overflow-hidden border border-gray-200">
            <div class="h-24 {shop.banner} relative">
              <div class="absolute -bottom-8 left-4">
                <div class="w-16 h-16 rounded-full bg-surface border-4 border-surface flex items-center justify-center text-3xl shadow-card">
                  {shop.logo}
                </div>
              </div>
              {#if shop.newProducts > 0}
                <span class="absolute top-3 right-3 px-2 py-1 bg-primary text-white text-xs font-medium rounded-full">
            ...
            <div class="pt-10 pb-4 px-4">
              <div class="flex items-center gap-2 mb-2">
                ...
                <span class="text-xs text-success font-medium">★ {shop.trustScore}%</span>
              </div>
              <Button variant="outline" size="sm" href="/shop/{shop.name.toLowerCase().replace(' ', '-')}" class="w-full">
                Visit Shop
              </Button>
            </div>
          </Card>
  ...
  <section class="mb-12 animate-fade-in" style="transition-delay: 200ms">
    <h2 class="text-xl font-bold text-text-main mb-6">Recently Purchased From</h2>
    
    <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
      {#each recentVendors as vendor, i}
        <Card hover={true} padding="md" className="text-center border border-gray-200">
          ...
          <Button variant="outline" size="sm" class="w-full text-xs">Order Again</Button>
        </Card>
      {/each}
    </div>
  </section>
  ...
  <section class="animate-fade-in" style="transition-delay: 300ms">
    <div class="flex items-center justify-between mb-6">
      ...
      <a href="/shop" class="text-sm font-medium text-primary hover:underline">Browse All →</a>
    </div>
